...
export async function sellerCreateOfferAction(formData: FormData) {
  ...
  // Strict IDOR ownership check: ensure this product belongs to the seller
  const product = await prisma.product.findUnique({
    where: { id: productId },
  });

  if (!product || (product.sellerId !== session.user.id && session.user.role !== "ADMIN")) {
    throw new Error("Forbidden: You can only create offers for your own products.");
  }
...
export async function sellerUpdateOfferAction(offerId: string, formData: FormData) {
  const session = await auth();
  ...
  if (!offer || (offer.product.sellerId !== session.user.id && session.user.role !== "ADMIN")) {
    throw new Error("Forbidden: You do not own this product offer.");
  }
...
export async function sellerToggleOfferStatusAction(offerId: string) {
  const session = await auth();
  if (!session?.user?.id) throw new Error("Unauthorized");
...
export async function sellerDeleteOfferAction(offerId: string) {
  const session = await auth();
  if (!session?.user?.id) throw new Error("Unauthorized");
