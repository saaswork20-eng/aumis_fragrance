...
import { sellerUpdateOfferAction } from "../../actions";
import { ArrowLeft } from "lucide-react";

export const dynamic = "force-dynamic";

interface SellerEditOfferPageProps {
  params: Promise<{ id: string }>;
}

export default async function SellerEditOfferPage({ params }: SellerEditOfferPageProps) {
  const session = await auth();
  if (!session?.user?.id) redirect("/auth/login");

  const { id } = await params;

  // Strict ownership check: ensure the offer belongs to this seller's product
  ...
  const updateWithId = sellerUpdateOfferAction.bind(null, offer.id);
            ...
            <div className="flex gap-2 mb-2">
              {[5, 10, 15, 20, 25, 30].map((pct) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    const input = document.getElementById("sellerEditDiscountInput") as HTMLInputElement;
